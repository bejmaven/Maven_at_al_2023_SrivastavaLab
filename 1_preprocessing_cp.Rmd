...
This script is to preprocess scRNA-seq data that has been output from CellRanger. It begins with loading in the dataset, then creating a Seurat object. We then process this object (with sctransform, harmony, etc) to clean the data and prepare it for downstream data analysis. In addition, we remove non-cardiac cells that are likely contaminating cell populations in the differentiation.

# 1. Set up; Load Libraries

Load in data. GEO link will be made available upon publication.
...
# Get the path of your current open file and set as working directory.
# Option; you can choose to do this manually.
...
# Assign a name to attach as a prefix to all named objects and files in this document.
FileName = "cp_all"
...
# Overview check of Seurat object.
seurat_dataset 
...
classification.vec[classification.vec=="4"] <- "0_siRNA_control"
classification.vec[classification.vec=="5"] <- "0_siRNA_ISL1"
...
# 3. Perform QC and Filtering

```{r}

# Visualize and save QC features of the data before further filtering.
pdf(paste("../analyses/QC/", FileName, "_raw_overview.pdf", setp = ""))
VlnPlot(seurat_dataset, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
dev.off()

# Now we will filter based on these QC metrics, using standard filtering for this cell type.

# 1. Filter cells by the percent.mito metadata we set earlier.
seurat_dataset <- subset(seurat_dataset, subset = percent.mt < 10)

# 2. Filter cells by nCount_RNA (this is UMI counts)
seurat_dataset <- subset(seurat_dataset, subset = nCount_RNA < 50000)

# 3. Filter cells by nFeature_RNA (how many measured genes)
...
pdf(paste("../analyses/QC/", FileName, "_filtered_overview.pdf", setp = ""))
VlnPlot(seurat_dataset, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
dev.off()

# Overview check of Seurat object after filtering.
seurat_dataset
# An object of class Seurat 
# 28164 features across 30059 samples within 1 assay 
# Active assay: RNA (28164 features, 0 variable features)

# Import genes for cell cycle regression.
cc.genes <- readLines(con = "../data/cell_cycle/cell_cycle_genes.csv")

# Split this list into markers of S phase and markers of G2/M phase.
s.genes <- cc.genes[1:45]
g2m.genes <- cc.genes[46:100]

# Assign cell cycle scores with these markers.
seurat_dataset <- CellCycleScoring(seurat_dataset, s.features = s.genes, g2m.features = g2m.genes, set.ident = TRUE)

# Confirm this.
head(x = seurat_dataset@meta.data)

# Clean up environment
rm(cc.genes, s.genes, g2m.genes)

# Use SCTransform to normalize and scale data. Here we regress for cell cycle genes.
...
pdf(paste("../analyses/QC/", FileName, "_filtered_DimPlot.pdf", sep = ""))
DimPlot(seurat_dataset, reduction = "umap", label = T)
DimPlot(seurat_dataset, reduction = "umap", group.by = "gem.group")
dev.off()

# Save Seurat object before proceeding with batch correction.
saveRDS(seurat_dataset, file = paste("../data/RDS/", FileName, "cp_filtered.rds", sep = ""))
...
# At this point, there is no batch correction applied. If you like, you can explore the dataset as is, though batch correction is recommended.
...
# Save UMAP as part of QC.
pdf(paste("../analyses/QC/", FileName, "_batchCorr_DimPlot.pdf", sep = ""))
DimPlot(seurat_dataset, reduction = "umap", label = T)
DimPlot(seurat_dataset, reduction = "umap", group.by = "gem.group")
dev.off()

# Save Seurat object before proceeding with batch normalization.
saveRDS(seurat_dataset, file = paste("../data/RDS/", FileName, "_batchCorr.rds", sep = ""))

# if you need to come back to this point
seurat_dataset <- readRDS(file = paste("../data/RDS/", FileName, "_batchCorr.rds", sep = ""))

```

# 5. Quick Overview Analysis

```{r}
# This is to provide a general check of the data before proceeding with further analyses.

# UMAPs of several cardiac markers.
pdf(paste("../analyses/QC/", FileName, "_batchCorr_FeaturePlot_cardiac.pdf", sep = ""))
FeaturePlot(seurat_dataset,  features = "MYH6")
FeaturePlot(seurat_dataset,  features = "TNNT2")
dev.off()

# UMAP of an endoderm marker, the usual cell contaminate in cardiac differentiations.
pdf(paste("../analyses/QC/", FileName, "_batchCorr_FeaturePlot_endoderm.pdf", sep = ""))
FeaturePlot(seurat_dataset,  features = "HHEX")
dev.off()

# Heatmap of cardiac markers.
pdf(paste("../analyses/QC/", FileName, "_batchCorr_DoHeatmap_cardiac.pdf", sep = ""))
DoHeatmap(seurat_dataset, features = c("ACTN2", "TNNT2", "NKX2-5", "MYH6", "MYL7", "HAND1"))
dev.off()

# Overall, it looks good. Time to do further analyses of this.
```

# 6. sessionInfo

```{r}
# Use this command to capture all session info current as of these analyses.
sessionInfo()
